...
|
| Window approach
|   "Exactly 7 days" is implemented as a 24-hour window anchored on
|   yesterday-at-this-time so a single daily run never misses nor
|   double-fires.  Specifically:
|     changed_at BETWEEN (now - 8 days) AND (now - 7 days)
|   This gives a full 24-hour catch window regardless of when the cron
|   fires within the day.
|
| Idempotency
|   Before sending, we check whether a reminder_claim notification already
|   exists for this request_id.  Re-running the command after a crash is
|   therefore safe and will not double-notify the student.
|--------------------------------------------------------------------------
*/
...
class SendUnclaimedReminders extends Command
{
    ...
    {
        // 24-hour catch window: requests that transitioned to ReadyToClaim
        // between 8 and 7 days ago.
        ...
        $requests = DocumentRequest::query()
            ...
            // The request entered ReadyToClaim in the 7-day window.
            ...
            // Idempotency: skip if a reminder was already sent for this request.
