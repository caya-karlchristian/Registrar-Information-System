    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4 backdrop-blur-[2px]">
      <div className={`w-full max-w-180 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] overflow-hidden border ${isDark ? 'bg-[#242526] border-[#3e4042]' : 'bg-white border-[#800000]/20'}`}>
        ...
        <div className="px-8 py-8 space-y-6">
          <div className={`text-sm leading-relaxed space-y-4 ${isDark ? 'text-[#e4e6eb]' : 'text-[#4a0000]'}`}>
            <p>
          ...
          <div className="flex flex-col gap-3">
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                className={`w-4 h-4 cursor-pointer ${isDark ? 'accent-[#f5c542]' : 'accent-[#800000]'}`}
              />
              <span className={`font-bold text-sm ${isDark ? 'text-[#e4e6eb]' : 'text-[#800000]'}`}>
        ...
        <div className={`px-8 py-5 flex justify-end gap-4 ${isDark ? 'bg-[#1f1f1f]' : 'bg-gray-50'}`}>
          <button
            onClick={handleCancel}
            className={`text-xs font-bold uppercase tracking-widest hover:underline transition ${isDark ? 'text-[#f5c542]' : 'text-[#800000]'}`}
          >
            Cancel
          </button>
          <button
