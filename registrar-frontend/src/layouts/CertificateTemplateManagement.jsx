  ...
  return (
    <div
      className={`rounded-lg border-2 border-dashed p-3 text-center transition ${
        dragOver ? "border-[#7f3f33] bg-[#fff7f4]" : "border-gray-300 bg-white"
      }`}
      onDragOver={(event) => {
        if (disabled) return;
    ...
    >
      ...
      <p className="mt-1 text-xs text-gray-500">Drag and drop image file{multiple ? "s" : ""} here</p>
      <label
        className={`mt-2 inline-block rounded-md px-3 py-1.5 text-xs font-semibold text-white ${
          disabled ? "cursor-not-allowed bg-gray-400" : "cursor-pointer bg-[#5c2a21] hover:bg-[#492119]"
        }`}
      >
        Upload
        <input
...
const PreviewModal = ({ isOpen, onClose, layout, certName }) => {
  ...
  return (
    <div className="fixed inset-0 z-9999 bg-black/55 p-4 sm:p-8">
      <div className="mx-auto flex h-full max-w-6xl flex-col rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-200 p-4">
          <h3 className="text-base font-bold text-[#4b1f16]">Certificate Modal Preview</h3>
          <button
            onClick={onClose}
        ...
        <header className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                onClick={resetLayout}
                className="rounded-md border border-gray-300 bg-gray-100 px-3 py-2 text-sm font-semibold hover:bg-gray-200"
              >
                Reset Logos
              </button>
              <button
                ...
                className="rounded-md bg-yellow-400 px-3 py-2 text-sm font-semibold text-slate-900 transition hover:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-yellow-200 disabled:cursor-not-allowed disabled:bg-yellow-200"
              >
                {saving ? "Saving..." : hasPreviewDataUrl(layout) ? "Waiting for upload..." : "Save Layout"}
              </button>
              <button
                onClick={() => setShowModal(true)}
                className="rounded-md bg-[#5c2a21] px-3 py-2 text-sm font-semibold text-white hover:bg-[#4b2119]"
              >
          ...
          <aside className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                <p className="text-xs font-semibold uppercase text-gray-700">Header Logo Size</p>
                <label className="mt-2 block text-sm">
                  <span className="block text-gray-700">Size: {layout.headerLogoSize}px</span>
                  <input
                    type="range"
                    min="40"
                    max="220"
              ...
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                <p className="text-xs font-semibold uppercase text-gray-700">Footer Logo Size</p>
                <label className="mt-2 block text-sm">
                  <span className="block text-gray-700">Size: {layout.footerLogoSize}px</span>
                  <input
                    type="range"
                    min="24"
                    max="160"
              ...
              <div className="space-y-2">
                {layout.footerUrls.map((logoUrl, index) => (
                  <div key={`${logoUrl}-${index}`} className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                    <div className="mb-2 flex items-center justify-between">
                      <p className="text-xs font-semibold text-gray-700">Footer Logo {index + 1}</p>
                      <button
