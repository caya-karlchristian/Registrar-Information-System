...
test.describe('Alumni Request Form E2E Tests', () => {
  ...
  test.beforeEach(async ({ context, page }) => {
    ...
    // 2. Mock /api/me to return an Alumni user
    ...
    await page.route('**/api/certifications', async (route) => {
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify([
          { certificate_type_id: 10, certificate_name: 'Graduation Certification', access_id: 3 }
        ]),
      });
    });
    ...
    // 6. Mock other reference lookup endpoints
    ...
    await page.route('**/api/document-requests/verify-or', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          is_mock: true,
          suggestions: {
            documents: [{ document_type_id: 2, document_name: 'Honorable Dismissal', number_of_copies: 1 }],
            certificates: [],
            unresolved: [
  ...
  test('Walk through multi-step Alumni Request wizard and submit successfully', async ({ page }) => {
    // Navigate to alumni request page
