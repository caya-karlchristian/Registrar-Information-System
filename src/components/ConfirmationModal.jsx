...
import { ExclamationTriangleIcon, ArrowRightStartOnRectangleIcon } from '@heroicons/react/24/outline';
      ...
      <div className="w-100 h-80 bg-white rounded-2xl shadow-2xl flex flex-col items-center justify-between p-7 animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex gap-3 w-full">
          ...
          <button
            ...
            className={`flex-1 px-4 py-2 text-sm font-semibold text-white rounded-lg shadow-md transition-colors ${
              isDanger 
                ? 'bg-red-600 hover:bg-red-700' 
                : 'bg-blue-600 hover:bg-blue-700'
            }`}
          >
            {isDanger ? 'Delete' : 'Logout'}
          </button>
        </div>
      </div>
    </div>
